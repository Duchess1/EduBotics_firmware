...
void EduBoticsMPU6050::update(void) {
...
void EduBoticsMPU6050::MadgwickQuaternionUpdate(float ax, float ay, float az, float gx, float gy, float gz) {
...
void EduBoticsMPU6050::getYawPitchRoll(double *yaw_req, double *pitch_req, double *roll_req) {
  *yaw_req = yaw; 
  *pitch_req = pitch; 
  *roll_req = roll;
}
...
//===================================================================================================================
void EduBoticsMPU6050::getGres(void) {
  switch (Gscale)
  {
...
void EduBoticsMPU6050::getAres(void) {
  switch (Ascale)
  {
...
void EduBoticsMPU6050::readAccelData(int16_t * destination)
{
...
void EduBoticsMPU6050::readGyroData(int16_t * destination)
{
...
int16_t EduBoticsMPU6050::readTempData()
{
...
void EduBoticsMPU6050::LowPowerAccelOnlyMPU6050(void)
{
...
void EduBoticsMPU6050::initMPU6050(void)
{  
...
void EduBoticsMPU6050::calibrateMPU6050(float * dest1, float * dest2)
{  
...
void EduBoticsMPU6050::MPU6050SelfTest(float * destination) // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
{
   uint8_t rawData[4];
   uint8_t selfTest[6];
   float factoryTrim[6];
...
void EduBoticsMPU6050::writeByte(uint8_t address, uint8_t subAddress, uint8_t data) {
...
uint8_t EduBoticsMPU6050::readByte(uint8_t address, uint8_t subAddress) {
...
void EduBoticsMPU6050::readBytes(uint8_t address, uint8_t subAddress, uint8_t count, uint8_t * dest){  
  Wire.beginTransmission(address);   // Initialize the Tx buffer
  Wire.write(subAddress);            // Put slave register address in Tx buffer
  Wire.endTransmission(false);       // Send the Tx buffer, but send a restart to keep connection alive
  uint8_t i = 0;
